  ...
  implements Ingredients
{
  ...
  constructor(
    ...
    this.ingredient = INGREDIENTS.GOAT_EYES;
    this.quality = quality;

    this.bus = scene.gamebus.getBus();

    const patty = scene.make.image({ key: goatEyes.sprite });
    patty.setDisplaySize(55, 65);
    patty.setDisplayOrigin(155, 100);
    patty.setTint(goatEyes.tint[this.quality]);
    this.add(patty);
  ...
  setQuality(quality: QUALITY) {
    this.quality = quality;
    this.ingredientImage.setTint(goatEyes.tint[quality]);
    this.qualityIndicator.setQuality(quality);
  }

  cookTick() {
    this.cookedScore += 1;
    if (this.cookedScore > goatEyes.cookStepTime) {
      this.cook();
    }
  }
  cook() {
...
export const goatEyes = {
  name: "Goat eyes",
  sprite: RESOURCES.GOAT_EYES,
