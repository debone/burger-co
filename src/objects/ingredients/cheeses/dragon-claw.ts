  ...
  implements Ingredients
{
  ...
  constructor(
    ...
    this.ingredient = INGREDIENTS.DRAGON_CLAW;
    this.quality = quality;

    this.bus = scene.gamebus.getBus();

    const patty = scene.make.image({ key: dragonClaw.sprite });
    patty.setDisplaySize(55, 65);
    patty.setDisplayOrigin(55, 100);
    patty.setTint(dragonClaw.tint[this.quality]);
    this.add(patty);
  ...
  setQuality(quality: QUALITY) {
    this.quality = quality;
    this.ingredientImage.setTint(dragonClaw.tint[quality]);
  }

  cookTick() {
    this.cookedScore += 1;
    if (this.cookedScore > dragonClaw.cookStepTime) {
      this.cook();
    }
  }
  cook() {
