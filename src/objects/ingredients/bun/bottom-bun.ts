  ...
  implements Ingredients
{
  ...
  constructor(
    ...
    this.ingredient = INGREDIENTS.BOTTOM_BUN;
    this.quality = quality;

    this.bus = scene.gamebus.getBus();

    const patty = scene.make.image({ key: bottomBun.sprite });
    patty.setDisplaySize(55, 65);
    patty.setDisplayOrigin(55, 100);
    patty.setTint(bottomBun.tint[this.quality]);
    this.add(patty);
  ...
  setQuality(quality: QUALITY) {
    this.quality = quality;
    this.ingredientImage.setTint(bottomBun.tint[quality]);
    this.qualityIndicator.setQuality(quality);
  }

  cookTick() {
    this.cookedScore += 1;
    if (this.cookedScore > bottomBun.cookStepTime) {
      this.cook();
    }
  }
  cook() {
...
export const bottomBun = {
  name: "Bottom Bun",
  sprite: RESOURCES.BURGER_BOTTOM,
