  ...
  implements Ingredients
{
  ...
  constructor(
    ...
    this.ingredient = INGREDIENTS.SOUL_BUN_TOP;
    this.quality = quality;

    this.bus = scene.gamebus.getBus();

    const patty = scene.make.image({ key: soulBunTop.sprite });
    patty.setDisplaySize(55, 65);
    patty.setDisplayOrigin(55, 100);
    patty.setTint(soulBunTop.tint[this.quality]);
    this.add(patty);
  ...
  setQuality(quality: QUALITY) {
    this.quality = quality;
    this.ingredientImage.setTint(soulBunTop.tint[quality]);
    this.qualityIndicator.setQuality(quality);
  }

  cookTick() {
    this.cookedScore += 1;
    if (this.cookedScore > soulBunTop.cookStepTime) {
      this.cook();
    }
  }
  cook() {
...
export const soulBunTop = {
  name: "Soul top",
  sprite: RESOURCES.SOUL_BUN_TOP,
