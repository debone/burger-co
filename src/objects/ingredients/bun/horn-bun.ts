  ...
  implements Ingredients
{
  ...
  constructor(
    ...
    this.ingredient = INGREDIENTS.HORN_BUN_TOP;
    this.quality = quality;

    this.bus = scene.gamebus.getBus();

    const patty = scene.make.image({ key: hornBun.sprite });
    patty.setDisplaySize(55, 65);
    patty.setDisplayOrigin(55, 100);
    patty.setTint(hornBun.tint[this.quality]);
    this.add(patty);
  ...
  setQuality(quality: QUALITY) {
    this.quality = quality;
    this.ingredientImage.setTint(hornBun.tint[quality]);
    this.qualityIndicator.setQuality(quality);
  }

  cookTick() {
    this.cookedScore += 1;
    if (this.cookedScore > hornBun.cookStepTime) {
      this.cook();
    }
  }
  cook() {
...
export const hornBun = {
  name: "Soul top",
  sprite: RESOURCES.HORN_BUN_TOP,
